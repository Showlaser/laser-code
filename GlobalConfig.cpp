#include "GlobalConfig.h"
#include "Settings.h"

const String MainMenuName = "Main Menu";
const String ModeSelectMenuName = "Mode selection";
const String StandAloneMenuName = "Standalone mode";
const String ControllerMenuName = "Controller mode";
const String SettingsMenuName = "Settings";
const String ProjectionZoneMenuName = "Projection zone";
const String ControllerIpMenuName = "Controller IP";
const String SDCardMenuName = "SD card";
const String PlaySDFileMenuName = "Play SD file";
const String ExitMenuName = "Exit";

LaserMode CurrentLaserMode = LaserMode::NotSelected;
String SelectedSDCardFile = "";