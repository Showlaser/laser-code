...
WDT_FUNC void WDT_OPT::begin(WDT_timings_t config)
{
  ...
  if (_device == WDT3)
  {
    ...
    if (config.clock == LPO_CLK || config.clock == INT_CLK)
    { /* LPO_CLOCK & INT_CLOCK are 32KHz */
      ...
      if (config.timeout < highest_limit_without_prescaler)
      { /* prescaler not needed */
...
WDT_FUNC void WDT_OPT::reset()
{
  if (WDT3 == _device || EWM == _device)
    SCB_AIRCR = 0x05FA0004; /* WDT3 & EWM doesn't have a reset register, fall back to ARM */
